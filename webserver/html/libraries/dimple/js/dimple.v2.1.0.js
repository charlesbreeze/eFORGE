...
(function (context, dimple) {
    ...
    dimple._showPointTooltip = function (e, shape, chart, series) {
        ...
        // APR 12/10/17: We may need to override the fill variable, if we are working with histone markers that have a custom fill/stroke
        var fill = selectedShape.attr("stroke");
        var trueStyle = d3.select(shape).attr('style');
        if (trueStyle) {
          var trueStyleAttrs = trueStyle.replace(/\s+/g, '').trim().split(';').forEach(function(e) {
            var attrPair = e.split(':');
            if (attrPair && ((attrPair[0] == 'fill' && attrPair[1] != 'white') || (attrPair[0] == 'stroke' && attrPair[1] != 'grey'))) {
              fill = attrPair[1];
            }
          });  
        }
